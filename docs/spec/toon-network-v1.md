...
A lease's address is created before its workload starts and answers on the same `ssh_port` and `host_port`s the access details give, so a tenant dials it exactly as it would an IP. It is destroyed when the lease ends, however it ends (§6.7), and a provider that restarts MUST re-establish each live lease's address rather than publish a new one: a tenant was given that host at its spawn. A provider that cannot re-establish one SHOULD give the lease a fresh address rather than leave it unreachable; `status` (§6.5) is where the tenant reads whichever address is current. A `Stopped` primary (§7.1) keeps its address — stopping is not ending — and a Warm Standby's reservation has none until a Takeover starts its workload.
