...
- **A NIP-01 event, nothing more.** A Lease Request is an ordinary Nostr event of kind `4432` (`K_LEASE_REQUEST`, §3.1) signed by the tenant's Nostr key. Its `id` is the SHA-256 of the NIP-01 serialization `[0, <pubkey>, <created_at>, <kind>, <tags>, <content>]` (UTF-8, no whitespace, NIP-01 escaping), and `sig` is a BIP-340 Schnorr signature over that `id` by `pubkey`. Nothing TOON-specific is hashed or signed, so any NIP-01 library produces and verifies a Lease Request.
...
6. ~~**Hostnames and TLS.**~~ **Closed, 2026-09-17 (Milestone 5, #46).** Hostnames and TLS stay out of the provider protocol and belong to a **Workload Gateway** keyed by `workload_id` (ADR 0013, now *Accepted*), and that gateway is specified: **§12** says what one is and that it is party to no lease, how it finds its grants (§12.1), the canonical hostname it derives from a workload id (§12.2), what its error page tells a tenant (§12.3), how it resolves a workload across its Standby Set (§12.4), what a forwarded request carries (§12.5), how a readable `name` is served (§12.6), how it follows the workload through a Takeover, a cadence, and a grant that ran out or named somebody else (§12.7), and how it fronts a workload on a Hidden Provider and what that discloses (§12.8). The authority the gateway needed is the **Gateway Grant** (§3.1.3, kind `30438`), which `status` honours (§6.5) and whose `http_port` says which of a spawn's `ports` is the HTTP one — so nothing is told to a Workload Gateway out of band.
...
Beside the canonical hostname, which a workload always has (§12.2), a grant MAY carry a `name` (§3.1.3), and a gateway MAY serve the workload at `<name>.<gateway domain>` as well.
