...
| `member_unreachable` | A member that would answer for the workload could not be reached — its connector, or the address it gave (§12.4). |
...
3. **The running member is the one answering `state: "running"` with `access`.** A member answering `reserved`, `stopped` or an ending (§6.7), refusing the request, or not answering at all is simply **not the target**; none of those is an error, and a Standby Set is expected to answer mostly `reserved`. Where more than one member answers `running` — a Takeover settled at two places for an instant — the member earliest in `standby_set` is taken, so two gateways holding the same grant resolve the same way.
4. **The target is `access.host`, and the `host_port` of the entry in `access.ports` whose `container_port` equals the grant's `http_port`.** It is **not** the first port listed and **not** `ssh_port`. A workload commonly exposes several ports and the host ports are the provider's to choose; the grant carries `http_port` precisely so that a gateway has to guess nothing (§3.1.3). A member answering `running` with no entry for that container port is not a target either.

A resolution in which every member answered and none was running is `no_running_member`; one in which a member that might have answered could not be reached is `member_unreachable` (§12.3). A gateway SHOULD log which member said what, because a Standby Set answering `reserved` everywhere and a Standby Set nobody can reach look identical from outside.
